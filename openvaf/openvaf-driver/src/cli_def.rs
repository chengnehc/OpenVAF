...
pub fn main_command() -> Command {
    Command::new("openvaf")
        ...
        .author("Pascal Kuthe")
        .about("A Verilog-A compact model compiler.")
...
// TODO(JW): check permission here rather than leave it to LLVM?
fn output_file_path_arg(name: &'static str) -> Arg {
    let parse = |raw: &str| {
        ...
        if path.exists() {
            ...
            let parent = raw_path.parent().unwrap();
    ...
    Arg::new(name).value_name("FILE").value_hint(ValueHint::FilePath).value_parser(parse)
}
...
fn dump_json() -> Arg {
    flag(DUMP_JSON, DUMP_JSON).help("Abort after lowering and serialize MIR as json.")
}

/* Batch mode and Cache */

fn batchmode() -> Arg {
    flag(BATCHMODE, "batch")
        .short('b')
        .help("Enable batchmode compilation.")
