...
use mir::{
    Block, Function, Inst, InstructionData, Opcode, Unknown, Value, F_LOG10_E, F_ONE, F_TWO, F_ZERO,
};
use mir::{DataFlowGraph, SourceLoc};
...
impl<'a, 'u> DerivativeBuilder<'a, 'u> {
    pub fn new(
        ...
        let derivative_values: AHashMap<Operand, Value> = intern
            .unknowns
            .iter_enumerated()
            .map(|(unknown, &val)| ((val, unknown), F_ONE))
            .collect();

        // a known value means it is not defined by any instruction
    ...
    /// Get a convenience builder object to insert instructions.
    ...
    pub fn run(mut self, post_order: &[Block]) -> AHashMap<Operand, Value> {
    ...
    fn build_normal_inst_derivatives(
        ...
        for derivative in derivatives.iter() {
            ...
            bcache.resolved_derivatives.insert(
                Some(derivative),
                ResolvedDerivative { instrs: inst_start..inst_end, cache_instrs: cache.insts },
            );
        }
    ...
    fn ddx(
        ...
        for next_unknown in pos_unknowns.iter() {
            let val = if let Some(deriv) = higher_order_derivative {
                let deriv = self.intern.raise_order(deriv, next_unknown);
                self.derivative_of(arg, deriv)
            } else {
                self.derivative_of1(arg, next_unknown)
            };
            if val == F_ZERO {
                continue;
            }
            derivative = if derivative == F_ZERO { val } else { self.ins().fadd(derivative, val) }
        }
        ...
        for next_unknown in neg_unknowns.iter() {
            ...
            derivative = if derivative == F_ZERO {
                self.ins().fneg(val)
            } else {
                self.ins().fsub(derivative, val)
            }
        }
        derivative
    }
...
impl DerivativeBuilder<'_, '_> {
    ...
    fn derivative_of(&self, val: Value, id: Derivative) -> Value {
        Self::derivative_of_(self.intern, &self.derivative_values, val, id)
    }

    fn derivative_of1(&self, val: Value, unknown: Unknown) -> Value {
    ...
    fn derivative_of_(
        ...
        for unknown in intern.unknowns_rev(derivative) {
            let Some(derivative) = derivative_values.get(&(val, unknown)) else { return F_ZERO };
            val = *derivative;
        }
        val
    }
    ...
    fn inst_derivative(&mut self, inst: Inst, unknown: Unknown, cache: CacheData) {
        ...
        let arg_derivative = |sel: &mut DerivativeBuilder, i| {
            sel.derivative_of1(sel.func.dfg.instr_args(inst)[i], unknown)
        };
        ...
        let gen_mul_derivative =
            |sel: &mut DerivativeBuilder, mut lhs: Value, mut rhs: Value, cast: bool| {
                ...
                let sum1 = match dlhs {
                    ...
                    F_ONE => rhs,
                    _ if dlhs == lhs => res,
                    _ => sel.simplified_mul(lhs, dlhs, rhs, res),
                };
                let sum2 = match drhs {
                    F_ZERO => return sum1,
                    F_ONE => lhs,
                    _ if drhs == rhs => res,
                    _ => sel.simplified_mul(rhs, drhs, lhs, res),
                };
                if dlhs == F_ZERO {
                    return sum2;
                }
        ...
        let gen_div_derivative =
            |sel: &mut DerivativeBuilder, mut lhs: Value, mut rhs: Value, cast: bool| {
                ...
                let top = match drhs {
                    ...
                    F_ONE => lhs,
                    _ if drhs == rhs => {
                        // then it becomes f/g, which is res
                        if sum1 == res {
                            return F_ZERO;
                        }
                        res
                    }
                    _ => sel.ins().fmul(drhs, lhs),
                };
