...
pub struct CallbackFun<'ll> {
    ...
    /// outside of the arguments provided in Verilog-A. These arguments are always
    /// passed before any arguments specified in the CFG.
...
impl<'ll> crate::CodegenCx<'_, 'll> {
    pub fn const_callback(
        ...
        let fun = self.declare_internal_fn(&name, fun_ty);
        unsafe {
    ...
    /// A callback without return value.
    pub fn trivial_callback(&self, args: &[&'ll llvm::Type]) -> CallbackFun<'ll> {
        let name = self.local_callback_name();
