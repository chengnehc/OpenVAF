...
impl<'ll> OsdiCompilationUnit<'_, '_, 'll> {
    pub fn descriptor(
        ...
        unsafe {
            ...
            let noise_sources = self.noise_sources();
            ...
            OsdiDescriptor {
                ...
                access: self.access_fn_prototype(),
    ...
    pub fn jacobian_entries(&self, target_data: &TargetData) -> Vec<OsdiJacobianEntry> {
        ...
        module
            ...
            .map(|entry| {
                ...
                if is_const(module, entry, false) {
                    flags |= JACOBIAN_ENTRY_RESIST_CONST
                }
                if is_const(module, entry, true) {
    ...
    pub fn noise_sources(&self) -> Vec<OsdiNoiseSource> {
        self.module
            .dae
            .noise_sources
            .iter()
            .map(|source| {
                ...
                let name = self.cx.literals.resolve(&source.name).to_owned();
    ...
    pub fn param_opvar(&self) -> Vec<OsdiParamOpvar> {
        ...
        let inst_params = inst_data.params.keys().map(|param| match param {
            ...
            OsdiInstanceParam::User(param) => {
                ...
                let flags = param_ty_flag(&ty) | PARA_KIND_INST;
                OsdiParamOpvar {
        ...
        let model_params = model_data.params.keys().filter_map(|param| {
            ...
            let ty = param.ty(self.db);
            let flags = param_ty_flag(&ty) | PARA_KIND_MODEL;
        ...
        let opvars = inst_data.opvars.keys().map(|opvar| {
            ...
            let flags = param_ty_flag(&ty) | PARA_KIND_OPVAR;
            OsdiParamOpvar {
