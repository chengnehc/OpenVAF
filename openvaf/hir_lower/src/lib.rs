...
impl HirInterner {
    ...
    // FIXME(JW) this is to work around borrow checker, there may be better way to do this.
...
pub enum PlaceKind {
    ...
    /// A flag indicating whether a source branch is a potential source
...
/// Implicit equations that are caused by unlinearizable analog operators
/// such as ddt, idt and noise sources.
#[derive(Copy, Clone, PartialEq, Eq, Hash, PartialOrd, Ord)]
pub struct ImplicitEquation(u32);
impl_idx_from!(ImplicitEquation(u32));
impl_debug_display!(match ImplicitEquation {ImplicitEquation(id) => "inode{id}";});
...
impl_debug_display!(match LimitState {LimitState(id) => "lim_state{id}";});
