...
pub trait VecExtensions<T> {
    /// Grows the index vector so that it contains an entry for `elem`;
    /// if that is already true, then has no effect. Otherwise, inserts
    /// new values as needed by invoking `fill_value`.
...
impl<T> SliceExtensions<T> for [T] {
    ...
    fn pick_n_mut<const N: usize>(&mut self, indices: [usize; N]) -> [&mut T; N] {
        ...
        for (res, idx) in Iterator::zip(res.iter_mut(), indices) {
            // This is safe as we ensure the offset are unique and in-bounds before
